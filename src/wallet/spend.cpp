        ...
        bool sign)
{
    ...
    auto res = CreateTransactionInternal(wallet, vecSend, change_pos, coin_control, fee_calc_out, sign);
    ...
    if (txr_ungrouped->fee > 0 /* 0 means non-functional fee rate estimation */ && wallet.m_max_aps_fee > -1 && !coin_control.m_avoid_partial_spends) {
        ...
        auto res2 = CreateTransactionInternal(wallet, vecSend, change_pos, tmp_cc, fee_calc_out, sign);
        if (res2) {
...
bool FundTransaction(CWallet& wallet, CMutableTransaction& tx, CAmount& nFeeRet, int& nChangePosInOut, bilingual_str& error, bool lockUnspents, const std::set<int>& setSubtractFeeFromOutputs, CCoinControl coinControl)
{
    ...
    FeeCalculation fee_calc_out;
    auto res = CreateTransaction(wallet, vecSend, nChangePosInOut, coinControl, fee_calc_out, false);
    if (!res) {
        error = res.GetError();
        return false;
    }
